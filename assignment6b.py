...
with graph.as_default():
    ...
    with tf.variable_scope("LSTM_parameters"):
        ...
        # Stacked parameters along second axis=1
        sx = tf.concat([wix, wfx, wcx, wox], 1) #stacked inputs
        sh = tf.concat([wih, wfh, wch, woh], 1) #stacked hidden states
        sb = tf.concat([bi, bf, bc, bo], 1) #stacked biases
    ...
    def lstm_cell(xt, ht_1, Ct_1, name):
        ...
        with tf.variable_scope(name):
            smatmul = xt @ sx + ht_1 @ sh + sb
            it_mul, ft_mul, Ct_hat_mul, ot_mul = tf.split(smatmul, num_or_size_splits=4, axis=1)

            it = tf.sigmoid(it_mul)
            ft = tf.sigmoid(ft_mul)
            Ct_hat = tf.tanh(Ct_hat_mul)
            Ct = ft * Ct_1 + it * Ct_hat
            ot = tf.sigmoid(ot_mul)
            ht = ot * tf.tanh(Ct)
        return ht, Ct
